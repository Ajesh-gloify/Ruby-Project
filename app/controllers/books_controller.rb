class BooksController < ApplicationController
  ...
  def show
    ...
    @book = Book.left_joins(:user).select('books.id, books.title, books.publlished_at, books.language, users.name').where("books.id = #{ params[:id] }")
  ...
  def update
    ...
    BookUpdateMailer.with(user: @user).send_bookupdate_email.deliver_now
  ...
  def book_params
    params.require(:book).permit(:title, :publlished_at, :language, :user_id, :library_id)
  end
